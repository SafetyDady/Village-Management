...
const UserManagement = () => {
  ...
  const createUser = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Create user failed:', response.status, response.statusText);
        return;
      }
  ...
  const updateUser = async () => {
    ...
    if (!selectedUser || !selectedUser.id) {
      ...
      console.error('Update failed: No user selected');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          alert(`Error: User ID ${selectedUser.id} not found in database`);
          console.error('Update failed: User not found (404)');
        } else {
          alert(`Error: HTTP ${response.status} - ${response.statusText}`);
  ...
  const deleteUser = async (userId) => {
    ...
    if (!userId) {
      ...
      console.error('Delete failed: Invalid user ID');
      return;
    }
    ...
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            alert(`Error: User ID ${userId} not found in database`);
            console.error('Delete failed: User not found (404)');
          } else {
            alert(`Error: HTTP ${response.status} - ${response.statusText}`);
